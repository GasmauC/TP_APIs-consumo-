...
const API = 'https://www.omdbapi.com/?i=tt3896198&apikey=dbc98f16';
...
const moviesController = {
    ...
    'buscar': (req, res) => {
        ...
        db.Movie.findAll({
            ...
            // guardamos la informacion del buscador
            .then(async result => { 

                if(result.Respons === "True"){

                const {Title, Released, Genre, Awards, imdbRating,Runtime} =   result

                const awardsArray = Awards.match(/\d+/g)  // extrae solo los digitos
        ...
        /*fetch(`${API}&t=${titulo}`) // --> promesa. Titulo me viene del form de la vista!!
            .then(response => {
                return response.json()
            })
            .then(result => {
